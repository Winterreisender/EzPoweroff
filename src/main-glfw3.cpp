...
// Decide GL+GLSL versions
    ...
    // 创建窗口
    GLFWwindow* window = glfwCreateWindow(210, 45, "EzPptTimer", NULL, NULL); assert(window);
    ...
    ImFont* font = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\MSYH.ttc", 21.0f, NULL,
        io.Fonts->GetGlyphRangesChineseSimplifiedCommon()); IM_ASSERT(font != NULL);
    ...
    enum class TimerState { RUNNING,PAUSED,CANCELED } timerState = TimerState::RUNNING; //三个状态 , C++ 没有thread.cancel()
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // UI部分
        {
            ImGui::Begin("ImGUI APP",NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings);

            ImGui::SetNextItemWidth(90.0f);
            ImGui::InputText("##",timerCountBuf,32,ImGuiInputTextFlags_ReadOnly);
            ImGui::SameLine();
            printf("FPS: %.1f\r", ImGui::GetIO().Framerate);

            //鼠标悬停在窗口上时
            if(glfwGetWindowAttrib(window, GLFW_HOVERED))
            {
                setFpsByInterval(1); // 提高帧率
                clear_color = ImVec4(.8f,.8f,.8f,1.0f);
                ImGui::SameLine();
